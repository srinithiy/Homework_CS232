...
/* Insert a node with an address, addr at the end of the linked
    list, pNode. It is assumed that pNode is the head of the linked
    */
...
/* Returns the number of times that a term occurs at least once across all web
    pages. It is assumed that pNode is the head of the linked list. This function is recursive.
     */
...
/* Calculate the score for a web page. This is done by finding the tf-idf value
    ...
    is then set to this value. it is assumed that pNode is the head of the linked list. when the function is called, the array, atLeastOneOccurenceInDocumentArr should be filled with the number of times that a term occurs at least once across all web pages. This function is recursive.    
    */
    ...
    while (fscanf(file, "%s %d", url, &hopsLimit) == 2 && n < MAX_N)
    {
        ...
        while (1)
        {
            if (!contains(urlHead, url))
            {
                ...
                if (n == 1)
                { // manually initialize first node
